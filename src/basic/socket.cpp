...
bool Socket::getOption(int level, int option, void* result, size_t* len) {
  ...
  if (rt) {
    LOG_DEBUG_STREAM << "getOption sock=" << m_sock << " level=" << level << " option=" << option
...
bool Socket::setOption(int level, int option, const void* result, size_t len) {
  if (setsockopt(m_sock, level, option, result, (socklen_t)len)) {
    LOG_DEBUG_STREAM << "setOption sock=" << m_sock << " level=" << level << " option=" << option
                     << " errno=" << errno << " errstr=" << strerror(errno);
    return false;
  }
  return true;
...
Socket::ptr Socket::accept() {
  ...
  if (newsock == -1) {
    LOG_ERROR_STREAM << "accept(" << m_sock << ") errno=" << errno << " errstr=" << strerror(errno);
    return nullptr;
  }
...
bool Socket::bind(const Address::ptr addr) {
  ...
  if (addr->getFamily() != m_family) {
    LOG_ERROR_STREAM << "bind sock.family(" << m_family << ") addr.family(" << addr->getFamily()
                     << ") not equal, addr=" << addr->to_string();
    return false;
  }

  if (::bind(m_sock, addr->getAddr(), addr->getAddrLen())) {
    LOG_ERROR_STREAM << "bind error errrno=" << errno << " errstr=" << strerror(errno);
    return false;
  }
  getLocalAddress();
  return true;
...
bool Socket::connect(const Address::ptr addr, uint64_t timeout_ms) {
  ...
  if (addr->getFamily() != m_family) {
    LOG_ERROR_STREAM << "connect sock.family(" << m_family << ") addr.family(" << addr->getFamily()
                     << ") not equal, addr=" << addr->to_string();
    return false;
  }
  ...
  if (timeout_ms == (uint64_t)-1) {
    if (::connect(m_sock, addr->getAddr(), addr->getAddrLen())) {
      LOG_ERROR_STREAM << "sock=" << m_sock << " connect(" << addr->to_string()
...
bool Socket::reconnect(uint64_t timeout_ms) {
  if (!m_remoteAddress) {
    LOG_ERROR_STREAM << "reconnect m_remoteAddress is null";
    return false;
  }
...
bool Socket::listen(int backlog) {
  if (!isValid()) {
    LOG_ERROR_STREAM << "listen error sock=-1";
    return false;
  }
  if (::listen(m_sock, backlog)) {
    LOG_ERROR_STREAM << "listen error errno=" << errno << " errstr=" << strerror(errno);
    return false;
  }
  return true;
}
...
int Socket::send(const void* buffer, size_t length, int flags) {
  if (isConnected()) { return ::send(m_sock, buffer, length, flags); }
  return -1;
}
...
int Socket::recv(void* buffer, size_t length, int flags) {
  if (isConnected()) { return ::recv(m_sock, buffer, length, flags); }
  return -1;
}
...
Address::ptr Socket::getRemoteAddress() {
  ...
  if (getpeername(m_sock, result->getAddr(), &addrlen)) {
    LOG_ERROR_STREAM << "getpeername error sock=" << m_sock << " errno=" << errno
                     << " errstr=" << strerror(errno);
...
Address::ptr Socket::getLocalAddress() {
  ...
  if (getsockname(m_sock, result->getAddr(), &addrlen)) {
    LOG_ERROR_STREAM << "getsockname error sock=" << m_sock << " errno=" << errno
                     << " errstr=" << strerror(errno);
    return Address::ptr(new UnknownAddress(m_family));
  }
  if (m_family == AF_UNIX) {
...
std::ostream& Socket::dump(std::ostream& os) const {
  ...
  if (m_localAddress) { os << " local_address=" << m_localAddress->to_string(); }
  if (m_remoteAddress) { os << " remote_address=" << m_remoteAddress->to_string(); }
  os << "]";
  return os;
}
...
void Socket::newSock() {
  ...
  if (m_sock != -1) {
    ...
    LOG_ERROR_STREAM << "socket(" << m_family << ", " << m_type << ", " << m_protocol
                     << ") errno=" << errno << " errstr=" << strerror(errno);
  }
}
...
bool SSLSocket::loadCertificates(const std::string& cert_file, const std::string& key_file) {
  ...
  if (SSL_CTX_use_certificate_chain_file(m_ctx.get(), cert_file.c_str()) != 1) {
    LOG_ERROR_STREAM << "SSL_CTX_use_certificate_chain_file(" << cert_file << ") error";
    return false;
  }
  if (SSL_CTX_use_PrivateKey_file(m_ctx.get(), key_file.c_str(), SSL_FILETYPE_PEM) != 1) {
    LOG_ERROR_STREAM << "SSL_CTX_use_PrivateKey_file(" << key_file << ") error";
    return false;
  }
  if (SSL_CTX_check_private_key(m_ctx.get()) != 1) {
    LOG_ERROR_STREAM << "SSL_CTX_check_private_key cert_file=" << cert_file
                     << " key_file=" << key_file;
    return false;
  }
  return true;
