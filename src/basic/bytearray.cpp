...
bool ByteArray::isLittleEndian() const {
  return m_endian == BASIC_LITTLE_ENDIAN;
}

void ByteArray::setIsLittleEndian(bool val) {
  if (val) {
    m_endian = BASIC_LITTLE_ENDIAN;
  } else {
    m_endian = BASIC_BIG_ENDIAN;
  }
}
...
void ByteArray::writeFint16(int16_t value) {
  if (m_endian != BASIC_BYTE_ORDER) { value = byteswap(value); }
  write(&value, sizeof(value));
}
...
void ByteArray::setPosition(size_t v) {
  if (v > m_capacity) { throw std::out_of_range("set_position out of range"); }
  m_position = v;
  if (m_position > m_size) { m_size = m_position; }
  m_cur = m_root;
  while (v > m_cur->size) {
...
bool ByteArray::writeToFile(const std::string& name) const {
  ...
  if (!ofs) {
    LOG_ERROR_STREAM << "writeToFile name=" << name << " error , errno=" << errno
...
bool ByteArray::readFromFile(const std::string& name) {
  ...
  if (!ifs) {
    LOG_ERROR_STREAM << "readFromFile name=" << name << " error, errno=" << errno
                     << " errstr=" << strerror(errno);
    return false;
  }
