...
bool Address::GetInterfaceAddresses(
  ...
  try {
    for (next = results; next; next = next->ifa_next) {
      ...
      if (addr) { result.insert(std::make_pair(next->ifa_name, std::make_pair(addr, prefix_len))); }
    }
  } catch (...) {
    LOG_ERROR("Address::GetInterfaceAddresses exception");
    freeifaddrs(results);
    return false;
  }
  freeifaddrs(results);
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
  ...
  if (error) {
    LOG_ERROR("IPAddress::Create(%s, %d) error=%d errstr=%s", address, port, error,
              strerror(errno));
    return nullptr;
  }
...
IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
  ...
  if (result <= 0) {
    LOG_ERROR("IPv4Address::Create(%s, %d) rt=%d errno=%d errstr=%s", address, port, result, errno,
              strerror(errno));
    return nullptr;
  }
  return rt;
...
IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
  ...
  if (result <= 0) {
    LOG_ERROR("IPv6Address::Create(%s, %d) rt=%d errno=%d errstr=%s", address, port, result, errno,
              strerror(errno));
    return nullptr;
  }
  return rt;
...
UnixAddress::UnixAddress(const std::string& path) {
  ...
  if (m_length > sizeof(m_addr.sun_path)) { throw std::logic_error("path too long"); }
