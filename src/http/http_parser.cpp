...
static Basic::ConfigVar<uint64_t>::ptr g_http_request_buffer_size = Basic::Config::Lookup(
    "http.request.buffer_size", (uint64_t)(4 * 1024), "http request buffer size");

static Basic::ConfigVar<uint64_t>::ptr g_http_request_max_body_size = Basic::Config::Lookup(
    "http.request.max_body_size", (uint64_t)(64 * 1024 * 1024), "http request max body size");

static Basic::ConfigVar<uint64_t>::ptr g_http_response_buffer_size = Basic::Config::Lookup(
    "http.response.buffer_size", (uint64_t)(4 * 1024), "http response buffer size");

static Basic::ConfigVar<uint64_t>::ptr g_http_response_max_body_size = Basic::Config::Lookup(
...
void on_request_fragment(void* data, const char* at, size_t length) {
  LOG_INFO_STREAM << "on_request_fragment:" << std::string(at, length);
  HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
...
void on_request_version(void* data, const char* at, size_t length) {
  ...
  if (strncmp(at, "HTTP/1.1", length) == 0) {
    ...
    LOG_WARN_STREAM << "invalid http request version: " << std::string(at, length);
...
void on_request_http_field(void* data, const char* field, size_t flen, const char* value,
  ...
  HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
  if (flen == 0) {
    LOG_WARN_STREAM << "invalid http request field length == 0";
...
void on_response_version(void* data, const char* at, size_t length) {
  ...
  if (strncmp(at, "HTTP/1.1", length) == 0) {
    ...
    LOG_WARN_STREAM << "invalid http response version: " << std::string(at, length);
    parser->setError(1001);
    return;
  }
...
void on_response_http_field(void* data, const char* field, size_t flen, const char* value,
  ...
  HttpResponseParser* parser = static_cast<HttpResponseParser*>(data);
  if (flen == 0) {
    LOG_WARN_STREAM << "invalid http response field length == 0";
    // parser->setError(1002);
    return;
  }
  parser->getData()->setHeader(std::string(field, flen), std::string(value, vlen));
...
size_t HttpResponseParser::execute(char* data, size_t len, bool chunck) {
  if (chunck) { httpclient_parser_init(&m_parser); }
