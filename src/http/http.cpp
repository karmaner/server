...
#define XX(num, name, string) \
  if (strcmp(#string, m) == 0) { return HttpMethod::name; }
  HTTP_METHOD_MAP(XX);
#undef XX
  return HttpMethod::INVALID_METHOD;
}
...
const char* HttpMethodToString(const HttpMethod& m) {
  ...
  if (idx >= (sizeof(s_method_string) / sizeof(s_method_string[0]))) { return "<unknown>"; }
...
std::ostream& HttpRequest::dump(std::ostream& os) const {
  ...
  if (!m_websocket) { os << "connection: " << (m_close ? "close" : "keep-alive") << "\r\n"; }
...
void HttpResponse::setCookie(const std::string& key, const std::string& val, time_t expired,
  ...
  ss << key << "=" << val;
  if (expired > 0) {
    ss << ";expires=" << Basic::time2str(expired, "%a, %d %b %Y %H:%M:%S") << " GMT";
  }
